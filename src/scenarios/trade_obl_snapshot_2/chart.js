...
export class Chart {
    ...
    initAxes() {
        ...
        this.yAxisLeft = new NumericAxis(this.wasmContext, { axisTitle: "asks", id: "yAxisLeft", stackedAxisLength: "10%", axisAlignment: EAxisAlignment.Bottom });
        ...
        this.yAxisRight = new NumericAxis(this.wasmContext, { axisTitle: "bids", id: "yAxisRight", stackedAxisLength: "10%", axisAlignment: EAxisAlignment.Bottom });
        ...
        this.yAxisVP = new NumericAxis(this.wasmContext, { axisTitle: "vp", id: "yAxisVP", stackedAxisLength: "10%", axisAlignment: EAxisAlignment.Bottom });
        this.sciChartSurface.yAxes.add(this.yAxisVP);
        this.yAxisVP.autoRange = EAutoRange.Always;
        this.yAxisVP.flippedCoordinates = true;
    ...
    init_series() {
        ...
        this.vpDataSeries = new XyDataSeries(this.wasmContext, {
            dataSeriesName: "VP",
            containsNaN: false, // set to false to avoid NaN values in the series,
            isSorted: false // set to true if the x-values are sorted, this can improve performance
        });
        this.vpSeries = new FastMountainRenderableSeries(this.wasmContext, {
            dataSeries: this.vpDataSeries,
            xAxisId: "xAxisLeft",
            yAxisId: "yAxisVP",
            stroke: "#FFFFFF",
            strokeThickness: 2,
        ...
        this.sciChartSurface.renderableSeries.add(this.vpSeries);
    ...
    update_obl_draw_accomulative(idx) {
        let levs = [];
        for (let i = this.obl.l1; i <= this.obl.l2; i++) {
            levs.push(i);
        }
        ...
        let asks = this.obl.asks[idx];
        let bids = this.obl.bids[idx];

        let asks_acc = [];
        let bids_acc = [];
        for (let i = 0; i < asks.length; i++) {
            if (i === 0) {
                asks_acc.push(asks[i]);
            }
            else {
                asks_acc.push(asks_acc[i - 1] + asks[i]);
            }
        }
        for (let i=bids.length - 1; i >= 0; i--) {
            if (i === bids.length - 1) {
                bids_acc.push(bids[i]);
            }
            else {
                bids_acc.push(bids_acc[bids_acc.length - 1] + bids[i]);
            }
        }   
        bids_acc.reverse();


        this.asksDataSeries.appendRange(levs, asks_acc);
        this.bidsDataSeries.appendRange(levs, bids_acc);

        this.obl_verticalLine.x1 = this.obl.t[idx]
        this.obl_verticalLine.isHidden = false;
    }

    update_obl_draw_moving_sum(idx, win) {
        let levs = [];
        for (let i = this.obl.l1; i <= this.obl.l2; i++) {
            levs.push(i);
        }
        ...
        for (let i = 0; i < asks.length; i++) {
            let sum = 0;
            for (let j = i; j < i + win && j < asks.length; j++) {
                sum += asks[j];
            }
            asks_acc.push(sum);
        }
    ...
    update_obl_draw_with_vp(idx, vp_duration=60000) {
        let levs = [];
        for (let i = this.obl.l1; i <= this.obl.l2; i++) {
            levs.push(i);
        }
        ...
        this.bidsDataSeries.clear();
        this.asksDataSeries.clear();
        this.vpDataSeries.clear();

        this.asksDataSeries.appendRange(levs, this.obl.asks[idx]);
        this.bidsDataSeries.appendRange(levs, this.obl.bids[idx]);
        let vpp = vp(this.obl.l1, this.obl.l2, this.tls.t, this.tls.l, this.tls.v, this.obl.t[idx] * 1000, this.obl.t[idx] * 1000 + vp_duration);
        this.vpDataSeries.appendRange(levs, vpp);
